...
void Scene::shapeTransformation(int type,float amt)
{
	...
	if(glm::abs(amt)>1e-5)
	{
		...
		case zGlobalRotate:
			if(pickedShape ==-1)
				globalSystemRot(amt,vec3(0,0,1),zAxis1);
			else
			{
				if(direction == -1 && pickedShape+2<shapes.size())
				{
					...
					shapes[pickedShape]->globalSystemRot(amt,vec3(0,0,1),zAxis1);
			}
		break;
		case xScale:
			if(pickedShape ==-1)
		...
		break;
		case xCameraLocalRotate:
			if (pickedShape == -1)
				myRotate(amt, vec3(1, 0, 0), zAxis1);
			else
				shapes[pickedShape]->myRotate(amt, vec3(1, 0, 0), zAxis1);
			break;
		case xCameraGlobalRotate:
			if (pickedShape == -1)
				globalSystemRot(amt, vec3(1, 0, 0), zAxis1);
			else
			{
				...
				if (direction == -1 && pickedShape + 2 < shapes.size())
				{
					...
					shapes[pickedShape]->globalSystemRot(amt, vec3(1, 0, 0), zAxis1);
			}
			break;
		default:
			break;
